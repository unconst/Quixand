...
class Sandbox:
	def __init__(self, template: Optional[str] = None, timeout: int = 300, metadata: Optional[dict] = None, env: Optional[dict] = None, workdir: Optional[str] = None, adapter: str | Adapter | None = None, resources: Optional[Resources] = None):
		self._cfg = Config()
		...
		sbx_cfg = SandboxConfig(image=image, timeout=timeout, env=env, workdir=workdir or self._cfg.workdir, metadata=metadata, resources=adapt_res)
